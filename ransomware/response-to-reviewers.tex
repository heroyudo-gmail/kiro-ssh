...
\begin{itemize}[leftmargin=1.4em, itemsep=1pt]
  ...
  \item Added an \textbf{explicit compression selection-rule table} (a rule-based decision tree by
        file type and size) for reproducibility.
  \item Added a \textbf{security analysis of the logical air-gap} discussing the window of
        vulnerability while the volume is mounted, together with its mitigations (strict IAM,
        read-only mount, immutable/WORM, minimal mount duration).
  \item Added an \textbf{RTO scalability test} (50~MB--1~GB) showing that recovery time grows
        approximately linearly, and mentioned the \textit{Silesia Compression Corpus} for further validation.
  \item Expanded future work beyond Random Forest.
  \item Finalized the list of 46 references (all cited, with DOIs where available).
  \item Requested the Editor's approval for a change in authorship: two additional co-authors
        who contributed substantially to the revision have been included, with the consent of all authors
        (see ``Additional journal requirements'' below).
\end{itemize}
...
optimal balance between storage efficiency and processing speed for low-RTO backup. We further added
(i) an \textbf{empirical entropy analysis} linking Shannon entropy per file type to the achieved
compression (e.g., low-entropy CSV/LOG at 4.6--5.2 bit/byte compress by 66--80\%, whereas
high-entropy JPG/XLSX near 8.0 bit/byte barely compress), and showing the sharp entropy rise upon
encryption ($\Delta H$ up to $+3.45$); and (ii) a \textbf{scalability test} demonstrating that the
recovery time (RTO) scales approximately linearly with dataset size ($\approx 0.0073$~s/MB; a 1~GB
dataset fully recovered in $\approx 7.7$~s with perfect integrity). Energy profiling and direct
comparison with deduplication/incremental backup remain listed as future work.}
\loc{Section~4 (statistical-analysis, baseline-comparison, entropy-analysis, and scalability subsections with tables) and Section~5 (remaining future work).}
...
\end{revbox}
\authors{Done. The reference list has been finalized to \textbf{46 references, all cited in the
text} (no orphan entries). Previously incomplete references were replaced with adequate indexed
sources and DOIs where available (e.g., Kapoor et al.\ \texttt{doi:10.3390/su14010008}; McIntosh et
al.\ \texttt{doi:10.1145/3691340}; Meland et al.\ \texttt{doi:10.1016/j.cose.2020.101762}). New
citations were also inserted in the State-of-the-Art paragraph, the backup-strategy discussion, and
the FPE section.}
\loc{References section (46 final entries) and in-text citations (Sections~1, 2, and~4).}
...
(a rule-based decision tree) driven by file type and size, and this is stated in the Introduction,
Method, and Conclusion. To ensure reproducibility by other researchers, we added an \textbf{explicit
selection-rule table} (e.g., CSV/LOG/TXT $\leq 100$~MB $\rightarrow$ Snappy/LZ4; large text
$\rightarrow$ Zstandard; JPG $\rightarrow$ lightweight algorithms; XLSX $\rightarrow$ LZ4/Zstandard
by size), together with the priority order and \textit{fallback} mechanism (Gzip as the safety net).}
\loc{Sections~1, 3 (compression implementation subsection, selection-rule table), and~5.}
...
% =====================================================================
\section*{Response to additional feedback (follow-up review)}
% =====================================================================
\reviewer{D1. Security analysis of the logical air-gap: discuss the ``window of vulnerability''
while the volume is mounted and its mitigations (IAM, read-only mount, WORM).}
\begin{revbox}\small
Discuss the logical air-gap security trade-off, specifically the risk of malware hijacking the
mounted volume, and how to mitigate it.
\end{revbox}
\authors{We added a dedicated subsection that honestly acknowledges a \emph{window of
vulnerability}: while the volume is mounted, it can technically be reached by other processes, so
\textit{memory-resident} ransomware could wait for a mount event. We state that logical isolation
substantially reduces---but does not eliminate---the attack surface, and then detail four
mitigations: (i) access restriction via IAM, (ii) read-only mount during recovery, (iii)
\textit{immutable}/WORM storage (object lock), and (iv) minimizing mount duration---while opening a
direction toward a \textit{zero-trust backup} architecture.}
\loc{Section~4 (security analysis of the logical air-gap and window of vulnerability subsection).}

\reviewer{D2. Scalability and generalizability: discuss behavior at multi-gigabyte workloads and the
possible use of a standard compression corpus (e.g., Silesia).}
\begin{revbox}\small
Add a scalability discussion for large workloads and propose a standard corpus for validation.
\end{revbox}
\authors{We added a scalability-test subsection measuring the backup--recovery cycle across several
dataset sizes (50~MB--1~GB), showing that RTO grows approximately linearly ($\approx 0.0073$~s/MB;
a 1~GB dataset is fully recovered in $\approx 7.7$~s with perfect integrity). We also mention the
\textit{Silesia Compression Corpus} as a reproducible avenue for further validation.}
\loc{Section~3 (dataset justification) and Section~4 (scalability-test subsection with table).}

\reviewer{D3. Translate figure labels into English to meet international publication standards.}
\begin{revbox}\small
Translate all figure labels into English.
\end{revbox}
\authors{For the English version of the manuscript, all figures (both diagrams and system-output
panels) were rebuilt with English labels from the same experimental data, so no Indonesian text
remains in any figure.}
\loc{All figures in the English manuscript version.}
...
and Data Availability sections, and included biographies for all authors. The revision is submitted
under the same manuscript ID.}

\reviewer{Request for approval: change in authorship.}
\authors{We respectfully request the Editor's kind consideration and approval for a change in the
authorship of this manuscript. The originally submitted version listed two authors. During the
revision, the scope of work expanded considerably---particularly the re-execution of all experiments
following the corrected \emph{compress-then-encrypt} workflow, the additional cross-platform
validation on AWS EC2 (Windows and Linux), the repeated-trial statistical analysis, and the
strengthened security discussion. These substantial additional contributions were made by two
colleagues who, in accordance with the ICMJE authorship criteria, have earned co-authorship.
We have therefore included them as the third and fourth authors. All four authors have read and
approved the revised manuscript and this change of authorship, and there is no dispute among the
authors regarding this matter. We fully understand that authorship changes require editorial
approval, and we sincerely apologize for any inconvenience. Should the Editor require any additional
documentation (for example, a signed authorship-change form or a statement of each author's
contribution), we will gladly provide it promptly. The individual contributions of all authors are
detailed in the Author Contributions (CRediT) statement of the revised manuscript.}
\loc{Author list, Author Contributions (CRediT) statement, and Biographies in the revised manuscript.}
