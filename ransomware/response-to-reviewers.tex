...
\end{revbox}
\authors{We report measured results per file type across all five algorithms and a combined
efficiency score. All figures are now referenced in the text before they appear. We candidly note
that repeated-trial statistics, CPU/RAM/throughput/energy profiling, and direct comparison with
deduplication/incremental backup are important extensions; these are listed explicitly as future
work rather than claimed. We prefer to state this limitation honestly than to present unverified numbers.}
\loc{Section~4 (compression results, all figure references) and Section~5 (future work).}
...
\end{revbox}
\authors{We strengthened the empirical basis by re-running the full experiment on two platforms
(Windows Server 2022 and Ubuntu 22.04 on AWS EC2), reporting per-type compression across five
algorithms and a cross-platform consistency table. Deeper statistical profiling (repeated trials,
CPU/RAM/throughput) is listed as future work rather than claimed without data.}
\loc{Section~4 (cross-platform subsection and consistency table) and Section~5 (future work).}
