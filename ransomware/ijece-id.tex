...
\begin{abstract}
\vspace{.3em}
Serangan ransomware menjadi ancaman serius terhadap ketersediaan dan integritas data pada lingkungan penyimpanan berbasis awan. Namun, sistem cadangan konvensional tetap rentan ketika repositori cadangan dapat diakses langsung dari sistem utama, sementara metode kompresi algoritma tunggal sering kali tidak efisien untuk tipe berkas yang heterogen. Penelitian ini mengusulkan sistem cadangan berbasis awan yang mengintegrasikan mekanisme cadangan \textit{air-gapped} dengan kompresi \textit{lossless} multi-algoritma adaptif untuk meningkatkan ketahanan data dan efisiensi penyimpanan. Sistem menggunakan verifikasi \textit{hash} SHA-256, analisis metadata, dan analisis struktur berkas untuk membedakan enkripsi cadangan yang sah dari modifikasi berkas akibat ransomware. Validasi dilakukan melalui model serangan tersimulasi yang terkontrol (tanpa mengeksekusi \textit{malware} nyata) pada sebuah \textit{dataset} sintetis berukuran terbatas yang terdiri atas 43 berkas dengan beragam tipe dan ukuran, mencakup kondisi normal dan kondisi terserang. Hasil eksperimen menunjukkan bahwa pada berkas teks ber-entropi rendah, kompresi mencapai penghematan penyimpanan hingga 89,5\% untuk berkas log dan 81,1\% untuk berkas CSV (Brotli), sementara Snappy dan LZ4 memberikan penghematan 66--80\% dengan waktu proses jauh lebih singkat; sebaliknya berkas JPG dan XLSX yang sudah terkompresi mendekati 0\% penghematan. Sistem yang diusulkan berhasil memulihkan seluruh 43 berkas terdampak tanpa galat (RTO 4,3--5,3 detik) dan mencapai \textit{False Positive Rate} (FPR) 0\% serta \textit{False Negative Rate} (FNR) 0\% pada lingkungan pengujian terkontrol. Secara keseluruhan, pendekatan yang diusulkan secara efektif meningkatkan keamanan, ketahanan, dan efisiensi penyimpanan sistem cadangan berbasis awan terhadap ancaman ransomware.
\end{abstract}
}
\parbox[l]{\textwidth}{%
...
Skala ancaman ini telah meningkat secara dramatis dalam beberapa tahun terakhir. Survei komprehensif terhadap lebih dari 50 artikel penelitian periode 2020--2025 melaporkan bahwa kerugian global akibat ransomware melampaui 30 miliar dolar AS pada 2023---meningkat sekitar 900\% dibanding 2015---dengan lebih dari 493 juta serangan terdeteksi pada 2022, rata-rata pembayaran tebusan mencapai 2,73 juta dolar AS pada 2024, dan rata-rata gangguan operasional selama 21 hari \cite{41}. Peninjauan mutakhir juga menegaskan bahwa ransomware kini bergeser ke era \textit{data exfiltration} dan bahwa banyak riset akademik masih tertinggal dari tren serangan nyata \cite{23}, \cite{26}. Ransomware modern telah berevolusi dari sekadar penguncian layar menjadi rantai serangan multi-tahap yang menerapkan \textit{double} dan \textit{triple extortion} (enkripsi data, kebocoran data, hingga serangan DDoS), diperparah oleh model \textit{Ransomware-as-a-Service} (RaaS) di \textit{darknet} \cite{40} dan pergeseran taktik yang terdokumentasi lintas periode pandemi \cite{33}, sehingga strategi pertahanan tidak lagi cukup hanya bertumpu pada pencegahan infeksi \cite{42}, \cite{46}. Untuk menata ragam pertahanan tersebut, literatur mengusulkan kerangka \textit{Detection--Avoidance--Mitigation} (DAM) yang mengklasifikasikan teknik pertahanan ke dalam tiga tahap: deteksi (analisis statis, dinamis, dan hibrida), penghindaran (kesadaran keamanan dan \textit{hardening}), serta mitigasi (pencadangan dan pemulihan) \cite{42}. Dalam taksonomi ini, penelitian yang diusulkan berfokus pada tahap \textbf{mitigasi dan pemulihan}, yakni memastikan data tetap tersedia dan pulih meskipun deteksi dini gagal, alih-alih bersaing pada akurasi deteksi berbasis \textit{machine learning}.
...
Keterkaitan antara efisiensi kompresi dan keamanan sesungguhnya berakar pada sifat \textit{entropi} data. Menurut teori informasi Shannon, batas kompresi \textit{lossless} sebuah berkas ditentukan oleh entropinya, $H(X) = -\sum_{i} p(x_i)\log_2 p(x_i)$, yang menyatakan rata-rata jumlah bit minimum untuk merepresentasikan setiap simbol. Data ber-entropi rendah (\textit{plaintext} seperti CSV dan log dengan pola berulang) memiliki redundansi tinggi sehingga dapat dikompresi secara signifikan, sedangkan data ber-entropi tinggi (berkas terenkripsi atau sudah terkompresi seperti JPG) mendekati keacakan sehingga rasio kompresinya sangat rendah. Karakteristik ini memunculkan dua implikasi yang saling terhubung: (i) pemilihan algoritma kompresi sebaiknya adaptif terhadap entropi/tipe berkas agar efisiensi optimal, dan (ii) lonjakan entropi yang tiba-tiba pada berkas yang semula ber-entropi rendah dapat menjadi indikator enkripsi tidak sah oleh ransomware. Namun, deteksi berbasis entropi memiliki celah: teknik \textit{Format-Preserving Encryption} (FPE) mempertahankan struktur berkas sehingga tidak menaikkan entropi secara mencolok dan dapat mengelabui detektor entropi. Dari sinilah muncul \textit{research gap} yang menjadi fokus penelitian ini: belum adanya kerangka kerja yang secara terpadu menggabungkan (a) optimasi penyimpanan melalui kompresi adaptif berbasis karakteristik berkas, (b) isolasi cadangan tahan-ransomware, dan (c) verifikasi integritas yang tidak bergantung semata pada entropi, melainkan pada \textit{hash} dan metadata, dalam satu sistem cadangan berbasis awan.
