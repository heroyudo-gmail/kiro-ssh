...
Perlu ditegaskan posisi penelitian ini di antara arus utama riset deteksi ransomware agar kontribusinya jelas. Sebagian besar penelitian mutakhir berfokus pada \textit{deteksi} berbasis pembelajaran mesin: pendekatan \textit{machine learning} klasik seperti XGBoost dan Random Forest dilaporkan mencapai akurasi hingga 98,9\% dengan seleksi fitur \cite{41}, model \textit{deep learning} (CNN, LSTM, dan hibrida CNN--LSTM) mencapai 96--99\% pada \textit{dataset} seimbang, dan pendekatan \textit{ensemble} bahkan menembus akurasi di atas 99\% \cite{41}. Sistem deteksi waktu-nyata seperti pemantauan instruksi AES-NI pada CPU mampu menghentikan serangan setelah hanya 10--15 berkas terenkripsi \cite{41}. Namun, seluruh pendekatan deteksi ini menghadapi tantangan yang konsisten dilaporkan literatur: keterbatasan \textit{dataset} yang usang, sulitnya deteksi lintas-platform, dan munculnya teknik penghindaran canggih seperti FPE, \textit{fileless}, dan \textit{Living-off-the-Land} \cite{41}. Berbeda dengan arus tersebut, penelitian ini \textbf{tidak bersaing pada akurasi deteksi}, melainkan memperkuat lapisan \textit{mitigation/recovery}---sejalan dengan kelompok studi strategi pencadangan (aturan 3-2-1, \textit{immutable}/WORM, \textit{air-gapped}) yang melaporkan tingkat pemulihan hingga 90\% tanpa membayar tebusan \cite{42}. Dengan demikian, sistem yang diusulkan bersifat komplementer terhadap detektor berbasis ML: ketika deteksi dini gagal, mekanisme integritas dan pemulihan terisolasi menjamin ketersediaan data.
