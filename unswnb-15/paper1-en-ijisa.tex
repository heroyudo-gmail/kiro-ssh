...
ability to generalize across networks is rarely evaluated honestly. This paper
...
comparable intersection feature set. We find that a single XGBoost model
...
\begin{highlights}
\item A statistically validated Semantic Feature Mapping aligns features across
extraction tools and exposes a TCP-window feature-extractor mismatch.
\item Single-source NIDS collapse cross-network (MCC $\approx 0$); the cause is
distribution shift, not feature insufficiency (joint training reaches near
in-domain).
\item Only $1\%$ target labels (or label-free cross-dataset mixup) recovers
cross-network MCC to $0.65$--$0.91$, explained via the Ben-David domain-adaptation
bound.
\item Real AWS traffic validation: FAR $<0.5\%$ under benign load; three-domain
distribution shift quantified via Wasserstein distance; few-shot restores AWS
detection.
\item A lightweight single XGBoost on 9 features supports edge/Green-AI deployment
($2.9$ MB, $\sim$439~$\mu$s/flow).
\end{highlights}

\begin{keyword}
network intrusion detection \sep cross-network generalization \sep semantic
feature mapping \sep domain adaptation \sep few-shot \sep real-traffic validation
\sep XGBoost
\end{keyword}
...
(analyzed in Section~\ref{sec:asymmetry}). The evaluation is limited to the binary
task. Using a single XGBoost is a deliberate design choice for edge efficiency
(Section~\ref{sec:edge}), not merely a limitation; exploring robust ensembles is
left as future work that must still respect deployment compute budgets.
...
foundation validated here. Other directions: (i) multi-class classification and
robust ensembles under edge compute budgets; (ii) stronger domain adaptation to
address the direction asymmetry; (iii) online/streaming continuous adaptation with
