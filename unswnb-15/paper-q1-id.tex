...
\begin{table}[h]
\centering
\caption{Profil efisiensi model (diisi dari pengukuran nyata). Placeholder
menandai angka yang akan dilaporkan setelah instrumentasi.}
\label{tab:efficiency}
\begin{tabular}{lc}
\toprule
Metrik & Nilai \\
\midrule
Ukuran model biner (Model A, 9 fitur) & \texttt{[TBD]} KB \\
Rata-rata latensi inferensi per \emph{flow} & \texttt{[TBD]} $\mu$s \\
\emph{Throughput} (flow/detik, 1 vCPU) & \texttt{[TBD]} \\
\bottomrule
\end{tabular}
\end{table}

\noindent Angka pada Tabel~\ref{tab:efficiency} akan diisi dari pengukuran nyata
(bukan estimasi) sebagai bukti kesiapan \emph{edge}.
...
(\emph{clip} non-negatif, pembulatan paket, penegakan $sbytes\!\ge\!spkts$,
penyetelan ulang rata-rata, dan proyeksi \emph{add-only}) di ruang asli sebelum
dipetakan kembali ke ruang ter-\emph{z-score}.
